...
AOS.init({
  ...
  offset: 0,          // trigger a bit earlier
  duration: 400,       // was ~1000ms via CSS, now ~0.4s
  easing: 'ease-out',
  once: true,          // don't re-animate on scroll up
  mirror: false,
  disable: false       // keep on mobile (set true if you want to skip on phones)
});

// Ensure above-the-fold content is visible immediately (no need to scroll)
window.addEventListener('load', function () {
  ...
  // Force-add aos-animate for elements already in the viewport
...
tl.set(container,{autoAlpha:1});tl.from(container,0.75,{xPercent:-100,ease:Power2.out});
tl.from(image,0.75,{xPercent:100,scale:1.12,delay:-0.70,ease:Power2.out});})
;}
